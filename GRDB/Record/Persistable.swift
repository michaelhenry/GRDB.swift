...
public struct PersistenceContainer {
    var storage: [String: DatabaseValueConvertible?]
    ...
    subscript(caseInsensitive column: String) -> DatabaseValueConvertible? {
        get {
            ...
            for (key, value) in storage where key.lowercased() == column {
                return value
            }
            return nil
        }
...
// TODO: remove this Sequence performance
extension PersistenceContainer : Sequence {
    /// TODO
    public func makeIterator() -> DictionaryIterator<String, DatabaseValueConvertible?> {
